...
int main(int argc, char* argv[])
{
  ...
  if (reader->CanReadFile(inputFilename.c_str()))
  {
    ...
    pcl::PointCloud<PointXYZ> pointCloudPos;
    vtkSmartPointer<vtkPoints> pts = polyData->GetPoints();
    pcl::PointCloud<PointXYZ> pointCloudColor;
    vtkSmartPointer<vtkDataArray> colorData = polyData->GetPointData()->GetScalars();
    for (int i = 0; i<pts->GetNumberOfPoints(); i++)
    {
      pointCloudPos.push_back(PointXYZ(pts->GetPoint(i)[0],pts->GetPoint(i)[1],pts->GetPoint(i)[2]));
      pointCloudColor.push_back(PointXYZ(colorData->GetTuple(i)[0],colorData->GetTuple(i)[1],colorData->GetTuple(i)[2]));
    }
    // Generate the data
    ...
    pointCloudPos.width = (uint32_t) pointCloudPos.points.size();
    pointCloudPos.height = 1;
    ...
    rangeImage.createFromPointCloud(pointCloudPos, angularResolution, maxAngleWidth, maxAngleHeight, sensorPose, coordinate_frame, noiseLevel, minRange, borderSize);
    float* imageArray = rangeImage.getRangesArray();
    for (int i= 0; i<40; i++)
    {
      for (int j= 0; j<34; j++)
        std::cout<< imageArray[i*34+j] << " ";
      std::cout<<std::endl;
    }
    rangeImage.recalculate3DPointPositions();
    std::vector<PointWithRange, Eigen::aligned_allocator<PointWithRange> > points = rangeImage.points;
    for (int i= 0; i<40; i++)
    {
      for (int j= 0; j<34; j++)
        std::cout<< points[i*34+j].data[0] << " "<<points[i*34+j].data[1]<<" "<<points[i*34+j].data[2]<<" ";
      std::cout<<std::endl;
    }
